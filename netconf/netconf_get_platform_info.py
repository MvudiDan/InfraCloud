...
m = manager.connect(
    ...
    device_params={"name": "iosxe"},  # "csr" also fine
    look_for_keys=False,
    allow_agent=False,
    timeout=30
)
...
"""
hostname_xml = netconf_get_config(hostname_filter)
print("Hostname:")
print(parseString(hostname_xml).toprettyxml())

# === [2] Platform / Version (OPER) ===
version_filter = """
<filter>
...
# === [4] Interfaces (CONFIG) — SWITCHED TO NATIVE MODEL (Option B) ===
native_intf_filter = """
<filter>
  <native xmlns="http://cisco.com/ns/yang/Cisco-IOS-XE-native">
    <interface/>
...
"""
interfaces_xml = netconf_get_config(native_intf_filter)
print("Interfaces:")
