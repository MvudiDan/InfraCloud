...
@app.route('/map')
def map_page():
    local_info = requests.get('https://ipinfo.io/json').json()
    loc = local_info.get('loc')
    local_coordinates = local_info.get('loc')
    return render_template('map_dynamic.html', local_coordinates=local_coordinates)
